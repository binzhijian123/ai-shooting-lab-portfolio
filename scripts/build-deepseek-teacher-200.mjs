...
const targetCount = Math.max(1, Number(args.count || 200));
...
const outputDir = resolveOutputDir(args.outputDir, path.join("data", "finetune", "shooting-deepseek-teacher-200"));
...
const goldenPromptBlock = useGoldenStandard
  ...
  : "";
const questions = buildTeacherQuestionBank(index, targetCount);
const accepted = [];
const rejected = [];
...
await writeFile(path.join(outputDir, "dataset_summary.json"), JSON.stringify({
  ...
  split_counts: { train: split.train.length, valid: split.valid.length, test: split.test.length },
  validation_policy: [
    "json_parse_required",
    "answer_required",
...
console.log(JSON.stringify({
  ok: accepted.length >= 5,
  schema_version: "build_deepseek_teacher_200_result.v1",
  dry_run: dryRun,
  output_dir: path.relative(root, outputDir),
...
function buildTeacherQuestionBank(index, targetCount) {
  ...
  for (const chunk of chunks) {
    if (questions.length >= targetCount * 2) break;
